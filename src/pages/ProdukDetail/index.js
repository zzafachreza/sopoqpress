...
export default function ProdukDetail({navigation, route}) {
  ...
  const openWhatsApp = () => {
    const message = `Halo, saya tertarik dengan buku "${
      product.judul_buku
    }" karya ${product.penulis} (${formatPrice(product.harga)}). Bisa dibantu untuk pembelian?`;
  ...
  const renderRatingStars = (rating) => {
    ...
    if (hasHalfStar) {
      stars.push(<Icon key="half" name="star-half" size={16} color="#f39c12" />);
    }
    
    const emptyStars = 5 - Math.ceil(rating);
    for (let i = 0; i < emptyStars; i++) {
      stars.push(<Icon key={`empty-${i}`} name="star-outline" size={16} color="#f39c12" />);
    }
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <View style={styles.infoContainer}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.detailsContainer}>
              ...
              <View style={styles.detailItem}>
                ...
                <Text style={styles.detailText}>{product.tahun_terbit}</Text>
              </View>
              <View style={styles.detailItem}>
          ...
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Ketentuan Pembelian</Text>
            <View style={styles.rulesContainer}>
              <View style={styles.ruleItem}>
                <Icon name="checkmark-circle" size={16} color={colors.primary} />
                <Text style={styles.ruleText}>Buku baru dan original</Text>
              </View>
              <View style={styles.ruleItem}>
                <Icon name="shield-checkmark" size={16} color={colors.primary} />
                <Text style={styles.ruleText}>Garansi penggantian jika rusak</Text>
              </View>
              <View style={styles.ruleItem}>
                <Icon name="car" size={16} color={colors.primary} />
                <Text style={styles.ruleText}>Gratis ongkir untuk pembelian di atas Rp 100.000</Text>
              </View>
              <View style={styles.ruleItem}>
                <Icon name="return-down-back" size={16} color={colors.primary} />
                <Text style={styles.ruleText}>Bisa dikembalikan dalam 7 hari</Text>
              </View>
            </View>
          </View>
        </View>
      ...
      <View style={styles.actionContainer}>
        ...
        <TouchableOpacity 
          style={[styles.buyButton, product.status !== 'tersedia' && styles.disabledButton]} 
          onPress={handleBuyNow}
          disabled={product.status !== 'tersedia'}>
          <Text style={styles.buyButtonText}>
            {product.status === 'tersedia' ? 'Beli Sekarang' : 'Habis'}
          </Text>
        </TouchableOpacity>
      </View>
    </View>
